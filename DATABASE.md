...
- **Status configuráveis**: status de negócio (orçamento, OS) usam `config_categories`
  ...
  futura. Valores essenciais marcados com `is_system = true`.
- **Numeração amigável**: `clients`, `vehicles`, `quotes` e `service_orders` recebem um código
  sequencial por organização (`document_sequences` + `fn_next_document_number`), além do `id`
  (UUID) — ex.: `ORC-2026-000123`, `CLI-000045`.
