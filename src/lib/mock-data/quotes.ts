...
import { calculateQuoteTotal, QUOTES, type QuoteStatus } from "@/lib/mock-data/quotes-data";
...
export const QUOTE_PIPELINE_COLUMNS: KanbanColumnData<QuoteCard>[] = QUOTE_STATUS_ORDER.map(
  (status) => {
    const meta = QUOTE_STATUS_META[status];
    const items = QUOTES.filter((quote) => quote.status === status).map((quote) => ({
      id: quote.id,
      code: quote.code,
